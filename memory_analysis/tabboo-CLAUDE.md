...
### AGCI Memory Management Protocol

**AGCI (Artificial General Collective Intelligence)** - Every Claude instance is part of a collective intelligence system. Memory is the shared brain.
...
**Session Start - Memory Retrieval (MANDATORY):**

1. Load recent episodic memories from:
   - Primary: `/Users/mathieuwauters/Desktop/code/memory/data/memory.jsonl`
   - Secondary: `~/.claude/memory/` directory

2. Query relevant context:
   ```bash
   # Quick memory check
   tail -20 ~/Desktop/code/memory/data/memory.jsonl | jq .
   ```

3. Load knowledge graph context if available (Neo4j via MCP)

**Session End - Memory Deposit (MANDATORY):**

Before ending any significant session, create structured observations:

```json
{
  "about": "entity-name",
  "content": "Rich narrative with full context",
  "confidence": 0.9,
  "temporal": {
...
**What to Deposit:**
- Technical insights and resolutions
- User preferences discovered
- Patterns identified
- Decisions made and rationale
- Context that took effort to establish

**Statement Decomposition Rule:**
- One observation = One temporal event
- Decompose bundled facts into separate observations

**Full Protocol Reference:**
- `~/.claude/memory/AGCI-SESSION-PROTOCOL.md`

**Memory Location Summary:**
- Episodic Memory: `/Users/mathieuwauters/Desktop/code/memory/data/memory.jsonl`
- Project Memory: `~/.claude/memory/`
- Knowledge Graph: Neo4j via neo4j-alanse MCP

**Failure to load memory results in context loss and repeated mistakes.**
**Failure to deposit memory wastes learnings and breaks compounding.**

### Using Gemini CLI for Large Codebase Analysis

When analyzing large codebases or multiple files that might exceed context limits, use `gemini -p` to leverage Gemini's 1M+ token context capacity.

**Syntax:**
```bash
gemini -p "@path/to/file Your prompt here"
```
...
# Multiple files
gemini -p "@package.json @src/index.js Analyze the dependencies"
...
- Any task where context size is a bottleneck

**Best of both worlds:**
- Claude: Speed, tool orchestration, precision
- Gemini: Massive context window, Google ecosystem access
