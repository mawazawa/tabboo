...
export const DocumentUploadPanel = ({ userId, onExtractionComplete }: DocumentUploadPanelProps) => {
  ...
  const processDocument = async (doc: UploadedDocument) => {
    try {
      ...
      const extractedData = await ocrEngine.extractAndStructure(doc.file);

      // Update progress
      updateDocument(doc.id, { progress: 75 });
      ...
      const { data: vaultData, error } = await supabase
        ...
        .insert({
          ...
          status: 'completed',
          fields_extracted: Object.keys(extractedData).filter(k => k !== 'confidence' && k !== 'extractionSource' && k !== 'extractedAt'),
          fields_count: Object.keys(extractedData).filter(k => k !== 'confidence' && k !== 'extractionSource' && k !== 'extractedAt').length
        })
        .select()
        .single();
      ...
      updateDocument(doc.id, {
        ...
        progress: 100,
        extractedData,
        type: (extractedData.documentType || 'unknown') as DocumentType
      });

      toast.success(`Extracted ${Object.keys(extractedData).length} fields from ${doc.file.name}`);
